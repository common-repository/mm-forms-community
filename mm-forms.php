...
define('MM_FORMS_PATH', plugin_dir_path(__FILE__));
define('MM_FORMS_FILE', plugin_basename(__FILE__));
define('MM_FORMS_FOLDER', dirname(MM_FORMS_FILE));
define('MM_FORMS_PATH_INCLUDES', MM_FORMS_PATH . 'includes/');

require_once(MM_FORMS_PATH_INCLUDES . 'functions.php');
...
class mm_forms {
	...
	function mm_forms() {
		...
		register_activation_hook(MM_FORMS_PATH . '/mm-forms.php', array(&$this, 'mm_forms_install'));
		add_action('init', array(&$this, 'export_file'), 11);
		add_action('init', array(&$this, 'import_file'), 11);
		add_action('init', array(&$this, 'mm_forms_tinymce_addbuttons'), 10);
	}

	function check_installation(){
		if ($_GET['page'] == MM_FORMS_FILE) {
			return;
		}
		...
		if (!is_dir(MM_FORMS_PATH . "upload/")){
			$mmf_dir_list2.= "<li>". MM_FORMS_PATH . "upload" . "</li>";
		}elseif (!is_writable(MM_FORMS_PATH . "upload")){
			$mmf_dir_list.= "<li>". MM_FORMS_PATH ."upload" . "</li>";
		}

		if (!is_dir(MM_FORMS_PATH . "upload/form-upload")){
			$mmf_dir_list2.= "<li>". MM_FORMS_PATH ."upload/form-upload" . "</li>";
		}elseif (!is_writable(MM_FORMS_PATH ."upload/form-upload")){
			$mmf_dir_list.= "<li>" . MM_FORMS_PATH . "upload/form-upload" . "</li>";
		}
		if (!is_dir(MM_FORMS_PATH ."upload/temp")){
			$mmf_dir_list2.= "<li>". MM_FORMS_PATH ."upload/temp" . "</li>";
		}elseif (!is_writable(MM_FORMS_PATH ."upload/temp")){
			$mmf_dir_list.= "<li>". MM_FORMS_PATH ."upload/temp" . "</li>";
		}
		if (!is_dir(MM_FORMS_PATH ."captcha/tmp")){
			$mmf_dir_list2.= "<li>". MM_FORMS_PATH ."captcha/tmp" . "</li>";
		}elseif (!is_writable(MM_FORMS_PATH ."captcha/tmp")){
			$mmf_dir_list.= "<li>". MM_FORMS_PATH ."captcha/tmp" . "</li>";
		}
		if (!is_dir(MM_FORMS_PATH ."exports")){
			$mmf_dir_list2.= "<li>". MM_FORMS_PATH ."exports" . "</li>";
		}elseif (!is_writable(MM_FORMS_PATH ."exports")){
			$mmf_dir_list.= "<li>". MM_FORMS_PATH ."exports" . "</li>";
		}
	...
	function export_file()
	{
		...
		{
			require_once(MM_FORMS_PATH_INCLUDES . 'class_actions.php');
	...
	{
		include_once(MM_FORMS_PATH_INCLUDES . 'mm-forms-install.php');
		require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
		install_mmf(MM_FORMS_PATH);
	}
	...
	{	
		...
		$this->recursive_remove_directory(MM_FORMS_PATH . 'upload');
		//delete_option('mmf');
	}
...
/*	function checkUserAuthentication($id) 
	...
	function load_plugin_textdomain() { // l10n
		load_plugin_textdomain('mmf', false, MM_FORMS_FOLDER . 'languages');
	}
...
/* Admin panel */
	...
	function add_pages() {
		...
		$base_url = get_site_url() . '/wp-admin/options-general.php';
	...
	function admin_head() {
		...
		if (isset($plugin_page) && $plugin_page == plugin_basename(__FILE__)) {
			$admin_stylesheet_url = plugins_url('css/admin-stylesheet.css', __FILE__ );
			echo '<link rel="stylesheet" href="' . $admin_stylesheet_url . '" type="text/css" />';						
			$admin_cal_url = plugins_url('includes/ui/', __FILE__ );
...
var _mmf = {
	...
	options: {
		dropdown_url: "<?php echo plugins_url( 'images/dropdown.gif' , __FILE__ ); ?>"
	}
};
//]]>
</script>
...
<script type='text/javascript' src='<?php echo $admin_cal_url; ?>jquery-1.4.2.min.js'></script>
<script type='text/javascript' src='<?php echo $admin_cal_url; ?>jquery-ui-1.8.2.custom.min.js'></script>
...
<?php
	...
	function option_page() {
		...
		if ('new' == $_GET['contactform']) 
		{
			...
			require_once(MM_FORMS_PATH_INCLUDES . 'admin-panel.php');
...
//		if($this->checkdateavailability($id) && $this->checkUserAuthentication($id))
		...
		{		
			$form .= '<form action="' . $url . '" method="post" class="mmf-form" enctype="multipart/form-data">';
		...
		foreach ($fes as $fe) {
			...
			if ('captchac' == $type) {
				...
				if ($filename = $this->generate_captcha($op))
					$captcha_url = plugins_url('captcha/tmp/' . $filename, __FILE__ );
    ...
    function wp_head()
    {
			$stylesheet_url = plugins_url('css/stylesheet.css', __FILE__ );
			$calendar_css = plugins_url('css/calendar.css', __FILE__ );
    ...
    function load_js() {
            global $pagenow;
    if (is_admin() && 'options-general.php' == $pagenow && false !== strpos($_GET['page'], MM_FORMS_FOLDER))
                    wp_enqueue_script('jquery');
            if (! is_admin()) {
                    $jquery_form_script = plugins_url('js/jquery.form.js', __FILE__ );
                    wp_enqueue_script('jquery-form', $jquery_form_script, array('jquery'), '');

                    $javascript_url = plugins_url('js/mm-forms.js', __FILE__ );
                    $javascript_url_ajax = plugins_url('js/ajaxfileupload.js', __FILE__ );

                    $calendar_js = plugins_url('js/calendar.js', __FILE__ );
...
/* Processing form element placeholders */
	...
	function form_element_replace_callback($matches) {
		...
		switch ($type) {
			...
			case 'fileField':
				...
				$temp_url = plugins_url('upload/temp/', __FILE__);
				$html .= '
				
				<img  id="thumb_image-'.$name.'" src="' . plugins_url('images/transparent.gif', __FILE__ ) . '" />
				<iframe width="0" height="0" style="visibility: hidden">
				</iframe>
				<br>
				<iframe id="bfile" name="bfile" src="'. plugins_url('includes/ajaxfileupload.php?ufiletypes='.$this->get_uploadfiletypes($fid).'&maxfilesize='. $this->get_maxfilesize($fid).'&fieldname='.$name, __FILE__ ) . '&temp=' . urlencode($temp_url) . '" scrolling="no" frameborder="0" width="400" height="55"></iframe>';
			...
			case 'datefield':					
				...
				$calendar_icon = plugins_url('images/calendar.png', __FILE__);
	...
	function submit_replace_callback($matches) {
		...
		if (empty($value))
			$value = __('Send', 'mmf');
		$ajax_loader_image_url = plugins_url( 'images/ajax-loader.gif', __FILE__ );
	...
	function save_form_data($id, $form_data)
	{
		...
		foreach(array_keys($form_data) as $key)
		{
			...
			if(eregi("uploaded-file-", $key) and $arr_value != 'none') {
				...
				copy(MM_FORMS_PATH . 'upload/temp/' . $arr_value, MM_FORMS_PATH . 'upload/form-upload/' . $arr_value);
				unlink(MM_FORMS_PATH . 'upload/temp/' . $arr_value);
				
				// copying tumbnail (old code)
				//copy(MM_FORMS_PATH . 'upload/temp/thumb_' . $arr_value, MM_FORMS_PATH . 'upload/form-upload/thumb_' . $arr_value);
				//unlink(MM_FORMS_PATH . 'upload/temp/thumb_' . $arr_value);
				
				//$arr_value  = '<a href="' . plugins_url('upload/form-upload/'. $arr_value, __FILE__) . '">'.$f_name.'</a>'; 
			}
	...
	function mm_forms_tinymce_addplugin($plugin_array) {
		$plugin_array['mm_forms'] = plugins_url('tinymce/plugins/mm-forms/editor_plugin.js', __FILE__);
		return $plugin_array;
	}
	...
	function mm_plugin_links($links, $file) {
	    ...
	    if ($file == $this_plugin){
	        $settings_link = '<a href="admin.php?page=' . MM_FORMS_FILE . '">'.__("Settings").'</a>';
    ...
    wp_create_nonce( plugin_basename(__FILE__) ) . '" />';
        ...
        if (!$results) {
                ...
                foreach ( $results as $res ) {
						...
						<a href="' . get_bloginfo('url') . '/wp-admin/options-general.php?page=' . MM_FORMS_FILE . '&action=viewDetail&id=' . $res->fk_form_joiner_id . '">
						<img src="' . plugins_url('images/view.png', __FILE__) . '" />
						</a>
						</th></tr>';
                }
        }
...
if(isset($_REQUEST['action']) and $_REQUEST['action']== 'rss' && isset($_REQUEST['form_id']) ) {
    require_once(MM_FORMS_PATH_INCLUDES . "mm-forms-rss-feed.php");
    ...
    require_once(MM_FORMS_PATH_INCLUDES . "mm-forms-generate-rss-feed.php");
}
